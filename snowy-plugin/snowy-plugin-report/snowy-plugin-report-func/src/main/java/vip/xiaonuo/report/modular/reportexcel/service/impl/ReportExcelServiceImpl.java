package vip.xiaonuo.report.modular.reportexcel.service.impl;
...
import vip.xiaonuo.report.core.enums.ExportTypeEnum;
...
import vip.xiaonuo.report.modular.reportexcel.controller.dto.ReportExcelStyleDto;
import vip.xiaonuo.report.modular.reportexcel.mapper.ReportExcelMapper;
...
import vip.xiaonuo.report.modular.reportexcel.util.CellType;
import vip.xiaonuo.report.modular.reportexcel.util.XlsUtil;
...
//        reportExcelDto.setTotal(jsonObject.getJSONObject("rows").size());
    ...
    public void exportExcelCore(ReportExcelDto reportExcelDto)
    {
        ...
        // 将JSONObject对象转换为列表
        java.util.List<Integer> dataNumList = rowData.keySet().stream().map(Integer::parseInt).sorted().collect(Collectors.toList());
    ...
    private void analysisSheetCellData(JSONObject dbObject, String setParam) {
        ...
        if (dbObject.containsKey("celldata") && null != dbObject.get("celldata")) {
            java.util.List<JSONObject> celldata = new ArrayList<>();
    ...
    public List<JSONObject> getRightDynamicCell(JSONObject addCellData,Map<String,JSONObject> cellDataMap,CellType cellType){
        ...
        java.util.List<JSONObject> rightCells = new ArrayList<>();
