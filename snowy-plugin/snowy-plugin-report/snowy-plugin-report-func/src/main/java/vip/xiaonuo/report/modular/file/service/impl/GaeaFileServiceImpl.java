package vip.xiaonuo.report.modular.file.service.impl;
...
import vip.xiaonuo.report.modular.file.mapper.GaeaFileMapper;
...
@Slf4j
//@RefreshScope
public class GaeaFileServiceImpl extends ServiceImpl<GaeaFileMapper, GaeaFile> implements GaeaFileService {
    ...
    @Override
    public IPage<GaeaFile> pageList(GaeaFileParam param) {
        QueryWrapper<GaeaFile> queryWrapper = new QueryWrapper<>();
        if(ObjectUtil.isNotEmpty(param.getOrder())) {
            if("ASC".equals(param.getOrder())){
                queryWrapper.lambda().orderByAsc(GaeaFile::getUpdateTime);
            }
            if("DESC".equals(param.getOrder())){
                queryWrapper.lambda().orderByDesc(GaeaFile::getUpdateTime);
            }
        }
